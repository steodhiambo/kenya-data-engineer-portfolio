...
class MPesaETLPipeline:
    ...
    def validate_data(self, df):
        """
        Validate the extracted data against required schema
        ...
        if len(invalid_amounts) > 0:
            self.logger.warning(f"Found {len(invalid_amounts)} transactions with invalid amounts")
    ...
    def transform(self, df):
        ...
        def calculate_fees(amount):
            if pd.isna(amount):
                ...
                return round(amount * fee_rate_low, self.amount_precision)
            else:
                return round(amount * fee_rate_high, self.amount_precision)
        ...
        transformed_df['TransactionCategory'] = transformed_df['TransactionType'].apply(
            lambda x: 'Payment' if 'Pay' in x or 'Bill' in x 
                     else 'Transfer' if 'Send' in x or 'Money' in x 
                     else 'Deposit/Withdrawal' if 'Deposit' in x or 'Withdrawal' in x 
                     else 'Other'
        )
        
        # Add sender and receiver initials for privacy
        transformed_df['SenderInitials'] = transformed_df['TransSender'].apply(
            lambda x: ''.join([name[0] for name in str(x).split()]) if pd.notna(x) else 'UNKNOWN'
        )
